...
curl -s -H "Content-type: application/json" -X POST http://localhost:9200/_sql --data-binary @$DIR/vuln_score.json > $DIR/vuln_score_result.json
...
cp $DIR/insert_template.json $DIR/insert_vuln_score.json
...
cp $DIR/insert_template.json $DIR/insert_itsec_score.json
sed -i 's/%1/vuln_score/g' $DIR/insert_itsec_score.json
